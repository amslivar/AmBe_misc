...
void calibrate_Ge_v2(const string &inData, TString detName, TString calibSource){
    ...
    double m_c = 0.5793;
    double b_c = 0.5455;//2.668535;
    double m_s = 0.0001668; //slope of the energy vs sigma relation (ch/E) unit
    double b_s = 0.6225; // intercept of the nergy vs sigma relation (ch)
 ...
 if(calibSource == "Th228"){
   ...
   peak_s = 74.815*m_s + b_s;
   peak_c2 = 76.863*m_c + b_c;
   peak_s2 = 76.863*m_s + b_s;
   peak_c3 = 77.107*m_c + b_c;
   peak_s3 = 77.107*m_s + b_s;
   xlow = peak_c - 60.;
   xhigh = peak_c + 60.;
   TF1* fit75_77 = new TF1("fit75_77",triple_gaus_linear,xlow,xhigh,11);
   fit75_77->SetParNames("A_{Pb212_75}","#mu_{Pb212_75}","#sigma_{Pb212_75}","b","m","A_{Bi212_76}","#mu_{Bi212_76}","#sigma_{Bi212_76}","A_{Pb212_77}","#mu_{Pb212_77}","#sigma_{Pb212_77}");
   fit75_77->SetParameters(9e4,peak_c,peak_s,247,0.099,9e4,peak_c2,peak_s2,9e4,peak_c3,peak_s3);
   
   h1->Fit("fit75_77","E","",xlow,xhigh);
   h1->SetTitle("Pb212 75,77 and Bi212 76 keV peak; counts ; Channel");
   ...
   channel.push_back(fit75_77->GetParameter(1));
   channel_e.push_back(fit75_77->GetParError(1));
   sigma.push_back(fit75_77->GetParameter(2));
   sigma_e.push_back(fit75_77->GetParError(2));
   energy.push_back(74.815);
   
   channel.push_back(fit75_77->GetParameter(6));
   channel_e.push_back(fit75_77->GetParError(6));
   sigma.push_back(fit75_77->GetParameter(7));
   sigma_e.push_back(fit75_77->GetParError(7));
   energy.push_back(76.863);
   
   channel.push_back(fit75_77->GetParameter(9));
   channel_e.push_back(fit75_77->GetParError(9));
   sigma.push_back(fit75_77->GetParameter(10));
   sigma_e.push_back(fit75_77->GetParError(10));
   energy.push_back(77.107);
   
   
   
   output << 74.815 << "\t" << fit75_77->GetParameter(1) << "\t" << fit75_77->GetParError(1) << "\t" << fit75_77->GetParameter(2) << "\t" << fit75_77->GetParError(2) << endl;
   ...
   output << 77.107 << "\t" << fit75_77->GetParameter(9) << "\t" << fit75_77->GetParError(9) << "\t" << fit75_77->GetParameter(10) << "\t" << fit75_77->GetParError(10) << endl;
    ...
    h1->SetTitle("Pb212 115.183 keV peak; counts; ADC Channel");
    h1->GetXaxis()->SetRange(xlow,xhigh);
    c1->Update();
    c1->Print(pdfName.Data());
    //cout << fit242->GetParameter(1) << endl;
   ...
   fit238_240->SetParNames("A_{Pb212_238keV}","#mu_{Pb212_238keV}","#sigma_{Pb212_238keV}","b","m","I_{Relative Intensity Ratio}","#mu_{Ra224_240keV}","#sigma_{Ra224_240keV}","BR_{Pb212_238keV}/A_{Ra224_240keV}");
    ...
    xlow = peak_c - 10.;
    xhigh = peak_c + 10.;
    TF1* fit300 = new TF1("fit300",double_gaus_linear,xlow,xhigh,8);
    fit300->SetParNames("A","#mu","#sigma","b","m","A2","#mu2","#sigma2");
    fit300->SetParameters(8953.,peak_c,peak_s,3900.,-13.6,150,167.,0.69);
    fit300->FixParameter(7,0.69);
    h1->GetXaxis()->SetRange(0,0);
    ...
    fit510->SetParameters(6e3,peak_c,peak_s,355.,-2.494);
    h1->GetXaxis()->SetRange(0,0);
    ...
    fit583->SetParameters(5e4,peak_c,peak_s,2872,-7.5);//1669
    h1->GetXaxis()->SetRange(0,0);
    ...
    fit785->SetParameters(1622,peak_c,peak_s,262.5,-0.2344); //(500,2247.5,peak_s,247,0.099);
    h1->GetXaxis()->SetRange(0,0);
   ...
   //Tl208 860 kev peak
    ...
    fit893->SetParameters(507.,peak_c,peak_s,585.5,-0.875);
    h1->GetXaxis()->SetRange(0,0);
