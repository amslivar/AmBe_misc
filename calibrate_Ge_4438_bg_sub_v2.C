...
void calibrate_Ge_4438_bg_sub_v2(const string &inData, const string &inBkg, TString detName, TString calibSource){
  ...
  if (liveTimeBkg != 0) {
    scale = liveTimeTh / liveTimeBkg;
  } else {
    std::cerr << "Error: liveTimeBkg is zero, division by zero is not allowed." << std::endl;
  }
  

  TH1F *hdata = new TH1F("hdata","Calibration data",8192,-0.5,8191.5);
  for(int i=0;i<data.size();i++){
    hdata->SetBinContent(i+1,data[i]);
  }

  TH1F *hbkg = new TH1F("hbkg","Calibration data",8192,-0.5,8191.5);
  for(int i=0;i<data.size();i++){
    hbkg->SetBinContent(i+1,bkg[i]);
  }  
  
  TH1F *h_bkg_scaled = new TH1F("h_bkg_scaled","Calibration data",8192,-0.5,8191.5);
  for(int i=1; i<=8192; i++){
    h_bkg_scaled->SetBinContent(i, (hbkg->GetBinContent(i)*scale));
    h_bkg_scaled->SetBinError(i,TMath::Sqrt(hbkg->GetBinContent(i)*scale) );
  }

  TH1F *h1 = new TH1F("h1","Calibration data",8192,-0.5,8191.5);
  h1->GetXaxis()->SetTitle("ADC Channel");
  h1->GetYaxis()->SetTitle("Number of events / (1 channel)"); 
  for(int i=1; i<=8192; i++){
    h1->SetBinContent(i, (hdata->GetBinContent(i) - h_bkg_scaled->GetBinContent(i)));
    h1->SetBinError(i,TMath::Sqrt(hdata->GetBinContent(i) + h_bkg_scaled->GetBinContent(i)) );
  }
  ...
  hdata->SetLineColor(kPink);
  h_bkg_scaled->Draw("hist same");
  h_bkg_scaled->SetLineColor(kBlue);
  h1->SetTitle("Background Subtraction");
  TLegend *legend = new TLegend(0.7, 0.7, 0.9, 0.9);
  legend->AddEntry(h1, "Data - Scaled Background", "l");
  legend->AddEntry(hdata, "Data", "l");
  legend->AddEntry(h_bkg_scaled, "Scaled Background", "l");
  legend->Draw();
  h1->GetXaxis()->SetTitle("ADC Channel");
  h1->GetYaxis()->SetTitle("Number of events / (1 channel)"); 
  c3->SetLogy();
  ...
  h1->Write();
  TString pdfName = "pdf_results/" + detName + "_Calibration_with_"+ calibSource + "_" + inData.substr(inData.find_last_of('/') + 1) + "_nobkg_v2.pdf";
  ...
  c3->Print(pdfFileOpen.Data()); 
  TString rootName = "pdf_results/" + detName + "_Calibration_with_"+ calibSource + "_" + inData.substr(inData.find_last_of('/') + 1) + "_nobkg_v2.root";
  ...
  double m_c = 1.11;
  double b_c = 0.9;
  double m_s = 0.00037; //slope of the energy vs sigma relation (ch/E) unit
  double b_s = 1.33; // intercept of the energy vs sigma relation (ch)
